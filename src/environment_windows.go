...
func (env *environment) getWifiNetwork() (*wifiInfo, error) {
	...
	// list interfaces
	var interfaceList *WLANInterfaceList
	...
	infoSize := unsafe.Sizeof(interfaceList.WlanInterface[0])
	for i := 0; i < numberOfInterfaces; i++ {
		network := (*WLANInterface)(unsafe.Pointer(uintptr(unsafe.Pointer(&interfaceList.WlanInterface[0])) + uintptr(i)*infoSize))
		if network.isState != 1 {
			continue
		}
		return parseNetworkInterface(network, phClientHandle)
	}
	return nil, errors.New("Not connected")
}

func parseNetworkInterface(network *WLANInterface, clientHandle uint32) (*wifiInfo, error) {
	info := wifiInfo{}
	info.Interface = strings.TrimRight(string(utf16.Decode(network.strInterfaceDescription[:])), "\x00")

	// Query wifi connection state
	var dataSize uint16
	var wlanAttr *WLANConnectionAttributes
